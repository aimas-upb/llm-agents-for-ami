...
def test_focus_hub_update_delete():
    client = TestClient(appmod.app)
    assert client.post("/workspaces/ws/focus", json={}).status_code == 200
    assert client.post("/hub/", json={}).status_code == 200
...
def test_import_requires_token(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="HA_TOKEN env var required"):
        runpy.run_path(os.path.join(os.path.dirname(__file__), "..", "ygg_ha_adapter.py"), run_name="__main__")
...
async def test_shutdown_calls_resets(monkeypatch):
    ...
    class Closable:
        async def close(self):
            return None
    monkeypatch.setattr(appmod, "ha_client", Closable())
    monkeypatch.setattr(appmod, "ha_rest", Closable())
