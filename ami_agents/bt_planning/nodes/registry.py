...
Every node type (the built-ins plus any custom compute node) registers:

- a ``compile`` factory: ``(spec, compile_child, ctx) -> py_trees.behaviour.Behaviour``
...
``ctx`` is an optional caller-supplied context (the ``IRExecutor`` instance in
production) that factories may consult for environment-level state the spec
cannot carry — today only the TD-SOSA settling-time fallback for action nodes.
...
CompileFn = Callable[[dict, ChildCompiler, Optional[Any]], py_trees.behaviour.Behaviour]
...
def compile_node(
    ...
    return entry.compile(spec, compile_child, ctx)
...
def _compile_sequence(spec, compile_child, ctx):
...
def _compile_selector(spec, compile_child, ctx):
    children = [compile_child(c) for c in spec.get("children", [])]
    return py_trees.composites.Selector(name=_name(spec), memory=False, children=children)


def _compile_parallel(spec, compile_child, ctx):
    children = [compile_child(c) for c in spec.get("children", [])]
...
def _compile_action(spec, compile_child, ctx):
    settling = spec.get("settling_time_seconds")
    if ctx is not None and hasattr(ctx, "_settling_time_for_action"):
        settling = ctx._settling_time_for_action(spec["action_url"], settling)
    return ActionAffordanceNode(
        name=_name(spec),
        action_url=spec["action_url"],
        parameters=spec.get("parameters", {}),
        settling_time_seconds=float(settling) if settling else 0.0,
    )


def _compile_condition(spec, compile_child, ctx):
...
def _compile_wait_condition(spec, compile_child, ctx):
...
def _compile_compute(spec, compile_child, ctx):
...
def _validate_condition(spec, path, validate_child):
    ...
    if not url or not isinstance(url, str):
        errors.append(f"{path}: {spec.get('type', 'condition')} nodes require 'property_url'")
    if "expected_value" not in spec:
        errors.append(f"{path}: {spec.get('type', 'condition')} nodes require 'expected_value'")
    return errors
...
register_node_type("wait_condition", compile=_compile_wait_condition, validate=_validate_wait_condition)
